...
int main(const int argc, const char *argv[argc])
{
    ...
    multidim_iterator_t *const iter = malloc(multidim_iterator_needed_memory(3));
    TEST_ASSERTION(iter != NULL, "Failed to allocate memory for iterator");

    multidim_iterator_init(iter, 3, (const size_t[]){d1, d2, d3});
    ...
    for (size_t i = 0; i < d1 * d2 * d3; ++i)
    {
        ...
        multidim_iterator_advance(iter, 2, 1);
    }
    TEST_ASSERTION(multidim_iterator_is_at_end(iter), "Iterator is not done");
    TEST_ASSERTION(multidim_iterator_get_flat_index(iter) == d1 * d2 * d3, "End index was not what was expected");

    // Check that iteration over the second dimension
    multidim_iterator_set_to_start(iter);
    for (size_t i = 0; i < d1 * d2; ++i)
    {
        TEST_ASSERTION(multidim_iterator_get_flat_index(iter) == i * d3, "Unexpected index");
    ...
    {
        TEST_ASSERTION(multidim_iterator_get_flat_index(iter) == i * d2 * d3, "Unexpected index");
    ...
    for (size_t stride = 1; stride < 25; ++stride)
    {
        ...
        while (last_index < d1 * d2 * d3)
        {
            ...
            last_index += stride;
            multidim_iterator_advance(iter, 2, stride);
        }
        TEST_ASSERTION(multidim_iterator_is_at_end(iter), "Iterator is not done");
        TEST_ASSERTION(multidim_iterator_get_flat_index(iter) == d1 * d2 * d3, "End index was not what was expected");
    }
    ...
    for (size_t i = 0; i < d1 * d2 * d3; ++i)
    {
        TEST_ASSERTION(multidim_iterator_get_flat_index(iter) == d1 * d2 * d3 - i, "Unexpected index");
        multidim_iterator_recede(iter, 2, 1);
    }
    ...
    for (size_t i = 0; i < d1 * d2; ++i)
    {
        TEST_ASSERTION(multidim_iterator_get_flat_index(iter) == d1 * d2 * d3 - i * d3, "Unexpected index");
    ...
    for (size_t i = 0; i < d1; ++i)
    {
        TEST_ASSERTION(multidim_iterator_get_flat_index(iter) == d1 * d2 * d3 - i * d2 * d3, "Unexpected index");
    ...
    {
        multidim_iterator_set_to_end(iter);
        TEST_ASSERTION(multidim_iterator_get_flat_index(iter) == d1 * d2 * d3, "Iterator did not correctly reset");
        unsigned last_index = 0;
        while (last_index < d1 * d2 * d3)
        {
            TEST_ASSERTION(multidim_iterator_get_flat_index(iter) == d1 * d2 * d3 - last_index, "Unexpected index");
            last_index += stride;
            multidim_iterator_recede(iter, 2, stride);
        }
        TEST_ASSERTION(multidim_iterator_is_at_start(iter), "Iterator is not at the start");
        TEST_ASSERTION(multidim_iterator_get_flat_index(iter) == 0, "Start index was not what was expected");
    }
